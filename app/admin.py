...
async def new_event_2(message: Message, state: FSMContext):
    ...
    await message.answer('Введите небольшое описание для мероприятия(введенный текст появится в сообщении с выбором события)')
    ...
    try:
        async with aiosqlite.connect('db.sqlite3') as db:
            await db.execute('INSERT INTO events (title, description, image, about, date, place, price, price_rub, duration, how_to_get_there) VALUES (?,?,?,?,?,?,?,?,?,?)',
                         (title, description, image, about, date, place, price, price_rub, duration, how_to))
            await db.commit()
            await state.clear()
    except Exception as e:
