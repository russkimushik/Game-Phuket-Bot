...
async def get_event(callback: CallbackQuery):
    ...
    try:
        if image == 'NoneType':
            ...
            await callback.message.answer(f'<b>🔥{event_info.title}🔥</b>\n{event_info.about}\n<b>📅Когда?</b>\n{event_info.date}\n'
                                        f'<b>Где?</b>\n{event_info.place}\n\n<b>💰Стоимость участия: </b>{event_info.price} бат'
                                        f'\n<b>⏳ Продолжительность: </b>{event_info.duration}\n'
                                        f'\n<b>🚀 Как попасть?</b>\nНажимайте на кнопку "Записаться на мероприятие"', reply_markup=kb.in_event)
        else:
            await callback.message.delete()
    ...
    async def sign_1qua(message: Message, state: FSMContext):
        ...
        async def sign_no_pay(callback: CallbackQuery, state: FSMContext):
            await callback.message.answer('Наш менеджер свяжется с вами для записи')
            event_price = 0
